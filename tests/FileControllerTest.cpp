...
private slots:
  ...
  void testSaveAndLoadWithSingleAccount() {
    ...
    QCOMPARE(loadedOp->category(), QString("Food"));
  }
  ...
  void testSaveAndLoadSplitOperation() {
    ...
    categoryController->addCategory("Food", 200.0);
    categoryController->addCategory("Transport", 100.0);
    ...
    QCOMPARE(loadedAllocs[0].category, QString("Food"));
    QCOMPARE(loadedAllocs[0].amount, -100.0);
    QCOMPARE(loadedAllocs[1].category, QString("Transport"));
  ...
  void testSaveAndLoadCategories() {
    categoryController->addCategory("Food", 500.0);
    categoryController->addCategory("Transport", 200.0);
    categoryController->addCategory("Entertainment", 100.0);
  ...
  void testSaveAndLoadCategorizationRules() {
    categoryController->addCategory("Groceries", 300.0);
    categoryController->addCategory("Fuel", 150.0);
    ...
    QCOMPARE(rules[0]->category(), QString("Groceries"));
  ...
  */
  ...
  void testImportFromCsvWithoutCategories() {
    ...
    QCOMPARE(op->category(), QString());  // Empty category
  ...
  void testImportAppliesCategorizationRules() {
    ...
    QCOMPARE(op->category(), QString("Groceries"));
  }

  // ========== Missing Categories from Operations ==========

  void testLoadAddssMissingCategoriesFromOperations() {
    // Create a YAML file with operations referencing categories not in the category list
    QString filePath = tempDir->filePath("missing_cat.comptine");
    QFile file(filePath);
    QVERIFY(file.open(QIODevice::WriteOnly | QIODevice::Text));
    QTextStream out(&file);
    out << "categories:\n";
    ...
    out << "    budget_limit: 200.00\n";
    out << "accounts:\n";
    out << "  - name: Test Account\n";
    out << "    operations:\n";
    out << "      - date: 2025-01-15\n";
    out << "        amount: -50.00\n";
    out << "        description: Test\n";
    out << "        category: MissingCategory\n";
    file.close();

    // Load - should automatically add MissingCategory
    fileController->loadFromYamlFile(filePath);

    // Verify category was added
    QCOMPARE(categoryController->categoryCount(), 2);
    Category* missingCat = categoryController->getCategoryByName("MissingCategory");
    QVERIFY(missingCat != nullptr);
    QCOMPARE(missingCat->budgetLimit(), 0.0);
  }

  void testLoadAddsMissingCategoriesFromSplitAllocations() {
    // Test that categories in split allocations are also added
    QString filePath = tempDir->filePath("missing_split_cat.comptine");
    QFile file(filePath);
    QVERIFY(file.open(QIODevice::WriteOnly | QIODevice::Text));
    QTextStream out(&file);
    out << "categories: []\n";
    ...
    out << "      - date: 2025-02-01\n";
    out << "        amount: -100.00\n";
    out << "        description: Split Purchase\n";
    out << "        allocations:\n";
    out << "          - category: Cat1\n";
    out << "            amount: -60.00\n";
    out << "          - category: Cat2\n";
    ...
    // Both categories should have been added
    QCOMPARE(categoryController->categoryCount(), 2);
    QVERIFY(categoryController->getCategoryByName("Cat1") != nullptr);
    QVERIFY(categoryController->getCategoryByName("Cat2") != nullptr);
  }
